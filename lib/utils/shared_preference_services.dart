...
abstract class SharedPreferenceService{
  ...
  static Future<bool> getMobileToken() async => await getPreferenceBool(KYC_REQUESTED)??false;
  static void setMobileToken(bool value) async => await setBool(KYC_REQUESTED, value);

  static Future<bool> allowSmsNotification() async => await getPreferenceBool(ALLOW_SMS_NOTIFICATION)??false;
  static void setAllowSmsNotification(bool value) async => await setBool(ALLOW_SMS_NOTIFICATION, value);

  static Future<bool> getDiscoverable() async => await getPreferenceBool(DISCOVERABLE)??false;
  static void setDiscoverable(bool value) async => await setBool(DISCOVERABLE, value);


  static Future<LocalUser> getUser() async => LocalUser.formJson(jsonDecode(await getPreferenceString(USER_DETAILS) ?? ""));
  static void setUser(LocalUser user) async => await setString(USER_DETAILS, jsonEncode(user.toJson()));


  static Future<bool> getVerified() async => await getPreferenceBool(MOBILE_VERIFIED)??false;
  static void setVerified(bool value) async => await setBool(MOBILE_VERIFIED, value);
